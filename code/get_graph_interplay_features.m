...
function [features_All,Complete_feature_list] = get_graph_interplay_features(marker1, marker2, marker3)
...
%% First Feature Set (x and y coordinates of nuclei centroids)
x = [Graph1_cells_coordinates(:, 1); Graph2_cells_coordinates(:, 1); Graph3_cells_coordinates(:, 1)];
y = [Graph1_cells_coordinates(:, 2); Graph2_cells_coordinates(:, 2); Graph3_cells_coordinates(:, 2)];
...
% features from dekaunay triangulation
vfeature(1) = HH;
vfeature(2) = LL; 
vfeature(3) = MM;
vfeature(4) = HL;
...
feature_list(1) = {'HH Count'};
feature_list(2) = {'LL Count'};
feature_list(3) = {'MM Count'};
...
r = 0.10;
alpha = 0.43;
[VY, VX, x1, y1, edges] = construct_ccgs(all_centroids_coordinates, alpha, r);
edges = edges | edges';
...
% Removing Family 1
edges1 = edges;
for i=1:size(Graph1_cells_coordinates,1)
    ind = Graph1_cells_indices(i);
    edges1(ind,:) = 0;
    edges1(:,ind) = 0;
end
[numcomp1, group1] = graphconncomp(sparse(edges1));
[CCGfeatsM1, feature_list1] = cluster_graph_features(edges1, all_centroids_coordinates);
...
[CCGfeatsM2,feature_list2] = cluster_graph_features(edges2, all_centroids_coordinates);
...
[CCGfeatsM3,feature_list3] = cluster_graph_features(edges3, all_centroids_coordinates);
...
OneDArray2 = (nonzeros(OneDArray)');
MeanDist= mean(OneDArray2);
feats(1)= MeanDist;
...
LenDist= size(OneDArray2, 2);
feats(9)= LenDist;
feature_list4(9) = {'No. Nonezero Distances'};
LenVertDist= size(AdjMat2, 2);
