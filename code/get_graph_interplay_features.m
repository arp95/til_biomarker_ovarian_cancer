function [features_All,Complete_feature_list] = getGraphInterplayFeatures_v4(marker1, marker2, marker3)

% Here we should group the markers into target graphs
% In this code we always considered 3 groups


Graph1_cells_coordinates = marker1; % Associated with low rejection
Graph2_cells_coordinates = marker2; % Associated with active rejection
Graph3_cells_coordinates = marker3; % Myocyte coordinates
lineWidth=2.5;
...
%     d = d + 1;
    ...
    %new way of making the symetric matrix
...
feature_list(3) = {'MM Count'}; % Myocyte group interconnection
...
% to build a ccg global graph on all the lymphocytes, we use
% "construct_ccgs_optimized" function previously coded by German et al

%To remember where the nuclei masks are
% nuclei_mask_all = imread('D:\Sara\CCIPD\Cache\QmIF\QmIf_Cache\test2\H1TN3_Scan1_all.png');% all nuclei used Pdl1 test
% 
% main_mask=nuclei_mask_all;
r=0.10;
alpha=0.43;
% 
% bounds=bwnuclei2bounds(logical(main_mask));
% bounds=bounds';

% Make a table to include both lymphocytes and myocytes centroid
% coordinates
% we could find these coordinates using "bwboundaries" or "bwnuclei2bounds"
% but wanted to be able to use previously labled indices


% adding corresponding indices for myocytes


[VY,VX,x1,y1,edges] = construct_ccgs_optimized_sara(all_centroids_coordinates,alpha, r); % This version of the function works best with my variables

edges= edges | edges';

% To find how many connected components in graph we have we use:
[numcomp,group] = graphconncomp(sparse(edges));

% Removing Family 1
edges1 = edges;

for i=1:size(Graph1_cells_coordinates,1)%length(Graph1_cells_coordinates) I didn't use length because for Graph with only one coordinate it makes a mistake (returns 2 instead of 1)
    ind = Graph1_cells_indices(i);
    edges1(ind,:) = 0;
    edges1(:,ind) = 0;
end

[numcomp1,group1] = graphconncomp(sparse(edges1));
[CCGfeatsM1,feature_list1] = cluster_graph_features_optimized_QIF_Sara_v4(edges1,all_centroids_coordinates); % This function is updated by Sara


% Removing Family 2
edges2 = edges;

for i=1:size(Graph2_cells_coordinates,1)
    ind = Graph2_cells_indices(i);
    edges2(ind,:) = 0;
    edges2(:,ind) = 0;
end

[numcomp2,group2] = graphconncomp(sparse(edges2));
[CCGfeatsM2,feature_list2] = cluster_graph_features_optimized_QIF_Sara_v4(edges2,all_centroids_coordinates); % This function is updated by Sara


% Removing Family 3
edges3 = edges;

for i=1:size(Graph3_cells_coordinates,1)
    ind = Graph3_cells_indices(i);
    edges3(ind,:) = 0;
    edges3(:,ind) = 0;
end

[numcomp3,group3] = graphconncomp(sparse(edges3));

[CCGfeatsM3,feature_list3] = cluster_graph_features_optimized_QIF_Sara_v4(edges3,all_centroids_coordinates); % This function is updated by Sara 

% In function "cluster_graph_features_optimized_QIF_Sara", since column 20
% was always empty and 27,28 were always inf and 30 was always NaN, I
% removed these features from the original set
...
for i = 1:size(del,1)
    ...
    % Candidate for optimization (here we try to find the triangles)
    if ((ismember(del(i,1),Graph1_cells_indices) && ismember(del(i,2),Graph2_cells_indices)&& ismember(del(i,3),Graph3_cells_indices))...
            ...
            || (ismember(del(i,1),Graph1_cells_indices) && ismember(del(i,2),Graph3_cells_indices)&& ismember(del(i,3),Graph2_cells_indices)))
                ...
                %some_edge_keep = [some_edge_keep;some3];
