...
X = [bounds(:,1),bounds(:,2)];
D = squareform(pdist(X,'euclidean')) + eye(length(X));%Sara edited X' to X
...
feats{28} = sum(kn==2)/N;
feature_list(28) = {'percentage end nodes'};% Sara removed because empty
...
T = array2table(A2,'VariableNames',{'XCoord','YCoord'});% I build the table of coordinates based on the centroids
Dedge=Dedge|Dedge';% The adjacency matrix should be symmetric
G2=graph(Dedge,T);% Now G2 has also the coordinates of the vertices
